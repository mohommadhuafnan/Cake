...
import { showcaseCategories as fallback } from '../data/homeContent'

function mapShowcase(c) {
  return {
    id: c.id,
    title: c.title,
    subtitle: c.subtitle,
...
export default function CategoryShowcase() {
  const { t } = useLanguage()
  const cmsItems = useCms('showcase')
  const showcaseCategories = cmsItems.length > 0 ? cmsItems.map(mapShowcase) : fallback
