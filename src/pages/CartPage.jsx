      ...
      <div className="max-w-4xl mx-auto px-6 py-16">
        ...
        {items.length === 0 ? (
          ...
          <>
            <ul className="space-y-6 mb-12">
              {items.map((item) => (
                <li key={item.id} className="flex gap-6 p-4 border border-gray-100 fade-up">
                  <img src={item.image} alt={item.name} className="w-24 h-24 object-cover bg-ivory" />
            ...
            <div className="border-t border-gray-100 pt-6 fade-up space-y-4">
              <CouponApply />
              <div className="flex justify-between mb-2"><span>{t('cart.subtotal')}</span><span>{formatPrice(subtotal)}</span></div>
              {discount > 0 && <div className="flex justify-between mb-2 text-green-600"><span>{t('cart.discount')}</span><span>-{formatPrice(discount)}</span></div>}
              <div className="flex justify-between text-xl font-medium mb-8"><span>{t('cart.total')}</span><span className="text-gold">{formatPrice(total)}</span></div>
